...
- A route is named by its main road: the road its steps spend the most distance on, with abbreviations expanded ("Waiyaki Wy/A104" → "Waiyaki Way"). When two routes in the same response share a main road, the one with more distance on it keeps the name (so Google's route order does not matter) and the other takes its next-longest road that is not another name for the same stretch (Google lists aliases together, as in "Kisumu- Nairobi Rd/Waiyaki Wy/A104"). It falls back to the names in `description`, which alone is not enough: Google fills it with road numbers ("A104") and it varies between calls for the same route.
