...
- Sending: WhatsApp `whatsapp://send?phone=<number>&text=` link to the saved contact first, `sms:` fallback when WhatsApp can't be opened, system share sheet for groups. Not `wa.me`: it always opens, in the browser when WhatsApp is missing, so it can't tell the app to fall back. The app only ever opens a prefilled composer.
...
- Unverified, test early: that alternative routes carry traffic-aware durations for future departure times; WhatsApp link behaviour (`whatsapp://send?phone=<number>&text=` prefill, no group targeting) on a real phone; Haiku's Sheng quality — move to a larger model if it reads badly.
